...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(set, NULL, CMD_SID_OPTION_BLE_CFG_SET_DESCRIPTION,
		     cmd_sid_option_ble_cfg_set, CMD_SID_OPTION_BLE_CFG_SET_ARG_REQUIRED,
		     CMD_SID_OPTION_BLE_CFG_SET_ARG_OPTIONAL),
	SHELL_CMD_ARG(get, NULL, CMD_SID_OPTION_BLE_CFG_GET_DESCRIPTION,
		     cmd_sid_option_ble_cfg_get, CMD_SID_OPTION_BLE_CFG_GET_ARG_REQUIRED,
...
static int parse_ble_cfg_adv_params(const struct shell *shell, int argc, const char **argv, int base,
				   struct sid_ble_cfg_adv_param *adv)
{
	char *end;
	unsigned long v;

	if (base < argc && argv[base]) {
		v = strtoul(argv[base], &end, 0);
		if (end == argv[base] || v > UINT32_MAX) {
			shell_error(shell, "Invalid fast_interval");
			return -EINVAL;
		}
		adv->fast_interval = (uint32_t)v;
	}
	if (base + 1 < argc && argv[base + 1]) {
		v = strtoul(argv[base + 1], &end, 0);
		if (end == argv[base + 1] || v > UINT32_MAX) {
			shell_error(shell, "Invalid fast_timeout");
			return -EINVAL;
		}
		adv->fast_timeout = (uint32_t)v;
	}
	if (base + 2 < argc && argv[base + 2]) {
		v = strtoul(argv[base + 2], &end, 0);
		if (end == argv[base + 2] || v > UINT32_MAX) {
			shell_error(shell, "Invalid slow_interval");
			return -EINVAL;
		}
		adv->slow_interval = (uint32_t)v;
	}
	if (base + 3 < argc && argv[base + 3]) {
		v = strtoul(argv[base + 3], &end, 0);
		if (end == argv[base + 3] || v > UINT32_MAX) {
			shell_error(shell, "Invalid slow_timeout");
			return -EINVAL;
		}
		adv->slow_timeout = (uint32_t)v;
	}

	adv->type = AMA_SERVICE;
	adv->fast_enabled = adv->fast_interval > 0;
	adv->slow_enabled = (adv->slow_interval > 0 && adv->fast_timeout);

	return 0;
}

static int parse_ble_cfg_conn_params(const struct shell *shell, int argc, const char **argv, int base,
				    struct sid_ble_cfg_conn_param *conn)
{
	char *end;
	unsigned long v;
	...
	if (base < argc && argv[base]) {
		v = strtoul(argv[base], &end, 0);
		if (end == argv[base] || v > UINT16_MAX) {
			shell_error(shell, "Invalid min_conn_interval");
			return -EINVAL;
		}
		conn->min_conn_interval = (uint16_t)v;
	}
	if (base + 1 < argc && argv[base + 1]) {
		v = strtoul(argv[base + 1], &end, 0);
		if (end == argv[base + 1] || v > UINT16_MAX) {
			shell_error(shell, "Invalid max_conn_interval");
			return -EINVAL;
		}
		conn->max_conn_interval = (uint16_t)v;
	}
	if (base + 2 < argc && argv[base + 2]) {
		v = strtoul(argv[base + 2], &end, 0);
		if (end == argv[base + 2] || v > UINT16_MAX) {
			shell_error(shell, "Invalid latency");
			return -EINVAL;
		}
		conn->slave_latency = (uint16_t)v;
	}
	if (base + 3 < argc && argv[base + 3]) {
		v = strtoul(argv[base + 3], &end, 0);
		if (end == argv[base + 3] || v > UINT16_MAX) {
			shell_error(shell, "Invalid conn timeout");
			return -EINVAL;
		}
		conn->conn_sup_timeout = (uint16_t)v;
	}
	return 0;
}
...
int cmd_sid_option_ble_cfg_set(const struct shell *shell, int32_t argc, const char **argv)
{
	...
	if (end == argv[1] || !IN_RANGE(cfg_type_raw, SID_BLE_USER_CFG_ADV,
	...
	if (cfg.cfg_type == SID_BLE_USER_CFG_INACTIVITY_TIMEOUT) {
		...
		if (argc >= 3 && parse_ble_cfg_adv_params(shell, argc, argv, 2, &cfg.adv_param) != 0) {
			return -EINVAL;
		}
		if (argc >= 7 && parse_ble_cfg_conn_params(shell, argc, argv, 6, &cfg.conn_param) != 0) {
			return -EINVAL;
		}
	}
...
{
	CHECK_ARGUMENT_COUNT(argc, CMD_SID_OPTION_BLE_CFG_GET_ARG_REQUIRED,
			     CMD_SID_OPTION_BLE_CFG_GET_ARG_OPTIONAL);
...
int cmd_sid_sdk_config(const struct shell *shell, int32_t argc, const char **argv)
{
	...
	shell_info(shell, "SID_SDK_CONFIG_ENABLE_LINK_TYPE_2: %d",
		   IS_ENABLED(CONFIG_SIDEWALK_SUBGHZ_SUPPORT));
